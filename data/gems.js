/* ===================== 보석 데이터 =====================
   6종 × 5등급, 순수 스탯. (DESIGN 4.2, 수치 표는 GEMS.md)
   kind: mul = 배율(%), add = 가산. v[등급-1] = 효과량.
   등급당 가치는 합성 비용(2개→1개, 지수)을 따라 오른다. */
...
const FINAL_GRADE=6;                       // 최종 보석 — Ⅴ 둘을 전용 합성대에서 (DESIGN 4.4)
const GRADE_TXT=['Ⅰ','Ⅱ','Ⅲ','Ⅳ','Ⅴ','최종'];

/* 최종 보석의 오라 — 스탯은 Ⅴ와 같고 이것이 실질 가치다 */
const AURA={
...
/* 드랍 튜닝 — 홈은 시작 2칸뿐이다. 초반엔 귀하게, 후반엔 합성 재료로 쌓이게.
   4%→3%(2026-08-20, 마릿수 28→36)→1%(2026-08-21): 보석이 넘쳐 홈 선택이 고민이 아니게 됐다 — 1/3로.
   대장 확정 2개는 유지 — 클라이맥스 보상.
   rate: 잡졸 드랍 확률 / gradeUpPerWave: 웨이브당 +1단계 드랍 확률 증가 / bossGems: 대장 확정 개수 */
const GEM_DROP={rate:.01, gradeUpPerWave:.02, bossGems:2};
