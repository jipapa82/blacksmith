...
const UP=[
  ...
  {id:'efire', n:'화염 부여', max:3, r:0, when:'적중 시', ok:a=>elemOk(a,'fire'),
   d:a=>`이 무기로 때린 적에게 ${STATUS.burn.dur[a.elFire]}초 화상 — 초당 이 무기 공격력의 ${Math.round(STATUS.burn.dpsPct[a.elFire]*100)}% 피해. 다시 때리면 갱신${elemOf(a)?'':'<br>이 무기의 원소가 불로 정해진다'}`,
   f:a=>a.elFire++},
  ...
  {id:'ecold', n:'냉기 부여', max:3, r:0, when:'적중 시', ok:a=>elemOk(a,'cold'),
   d:a=>`이 무기로 때린 적을 ${STATUS.chill.dur}초 감속 (이동 -${Math.round(STATUS.chill.slow[a.elCold]*100)}%). 감속 중 ${STATUS.chill.hitsToFreeze}회 더 때리면 ${STATUS.chill.freezeDur[a.elCold].toFixed(1)}초 빙결 (풀린 뒤 4초 재빙결 면역)${elemOf(a)?'':'<br>이 무기의 원소가 얼음으로 정해진다'}`,
   f:a=>a.elCold++},
  ...
  {id:'leech', n:'피 먹는 홈',  max:4, r:0, when:'처치 시', ok:()=>1,
   d:a=>`이 무기가 적을 잡을 때마다 이 용병 체력 +1 (현재 +${a.leech}, 토파즈 흡혈과 합산)`, f:a=>a.leech+=1},
  {id:'repel', n:'밀치는 반격', max:3, r:0, when:'피격 시', ok:()=>1,
   d:a=>`이 용병을 때린 적이 ${60*(a.repel+1)}만큼 튕겨나고 공격 충전도 처음부터 — 파고드는 적을 떼어낸다`, f:a=>a.repel++},
  ...
  {id:'hone',  n:'날 세우기', max:4, r:0, when:'상시', ok:()=>1,
   d:a=>`이 용병 치명타 확률 +5%p (현재 ${Math.round(critOf(a)*100)}%)`, f:a=>a.crit+=.05},
  {id:'weight',n:'무게 중심', max:3, r:0, when:'상시', ok:()=>1,
   d:a=>`이 용병 치명타 피해 +25%p (현재 ${Math.round((a.critD+(a.gm?a.gm.critDmgAdd:0))*100)}%)`, f:a=>a.critD+=.25},
  {id:'plate', n:'덧댄 철판', max:3, r:0, when:'상시', ok:()=>1,
   d:()=>`이 용병 최대 체력 +12% — 고르는 즉시 늘어난 만큼 회복된다`,
   f:a=>{const b=maxHpOf(a);a.cardHpMul+=.12;a.hp+=maxHpOf(a)-b;}},

  /* ===== 희귀 — 무기 행동 ===== */
  {id:'dash',  n:'이어지는 질주', max:3, r:1, when:'기본 공격', ok:a=>a.trait==='assassin',
   d:a=>`침투자·대장이 없을 때의 질주가 ${a.dashN+2}명을 벤다 (현재 ${a.dashN}명) — 벤 수만큼 독이 발린다`, f:a=>a.dashN+=2},
  ...
  {id:'thorn', n:'가시 박기',   max:5, r:1, when:'피격 시', ok:a=>a.trait==='wall',
   d:a=>`이 용병을 때린 적에게 ${a.thorns+6} 반격 피해 (현재 ${a.thorns})`, f:a=>a.thorns+=6},
  {id:'bulk',  n:'거대한 방패', max:4, r:1, when:'상시', ok:a=>a.trait==='wall',
   d:()=>'이 용병이 적을 막아 세우는 범위 +15%', f:a=>a.blockR*=1.15},
  ...
  {id:'ambush', n:'기습',       max:4, r:1, when:'기본 공격', ok:a=>a.trait==='assassin',
   d:a=>`침투자·대장 일격 피해 ×${(a.ambush+.25).toFixed(2)} (현재 ×${a.ambush.toFixed(2)})`, f:a=>a.ambush+=.25},
  ...
  {id:'coldcut', n:'한파의 날', max:2, r:2, when:'상시', ok:()=>allies.some(x=>x.elCold),
   d:a=>`이 무기의 피해가 감속·빙결 상태의 적에게 +${Math.round(STATUS.syn.coldcut*100*(a.syColdcut+1))}% (누가 걸었든)`,
   f:a=>a.syColdcut++},
