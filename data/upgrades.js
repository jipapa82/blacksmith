...
const UP=[
  ...
  {id:'ecold', n:'냉기 부여', max:3, r:0, ok:a=>elemOk(a,'cold'),
   d:a=>`적중 시 ${STATUS.chill.dur}초 한기(이동 -${Math.round(STATUS.chill.slow[a.elCold]*100)}%). 한기 중 ${STATUS.chill.hitsToFreeze}회 적중 시 ${STATUS.chill.freezeDur[a.elCold].toFixed(1)}초 빙결${elemOf(a)?'':'<br>이 무기의 원소가 얼음으로 정해진다'}`,
   f:a=>a.elCold++},
  ...
  {id:'ambush', n:'기습',       max:4, r:1, ok:a=>a.trait==='assassin',
   d:a=>`암살 일격(침투자·대장) 피해 +25% (현재 ${Math.round(a.ambush*100)}%)`, f:a=>a.ambush+=.25},
