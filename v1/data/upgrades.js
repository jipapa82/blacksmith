...
const UP=[
  ...
  {id:'repel', vis:1, n:'밀치는 반격', max:3, r:0, when:'피격 시', ok:()=>1,
  ...
  {id:'dbl', vis:1,   n:'이중 타격',   max:3, r:0, when:'기본 공격', ok:()=>1,
  ...
  {id:'heavy', vis:1, n:'묵직한 손',   max:3, r:0, when:'적중 시', ok:()=>1,
  ...
  /* ===== 희귀 — 무기 행동 ===== */
  {id:'dash', vis:1,  n:'이어지는 질주', max:3, r:1, when:'기본 공격', ok:a=>a.trait==='assassin',
   d:a=>`침투자(전선을 넘어온 적)·대장이 없을 때의 질주가 ${a.dashN+2}명을 벤다 (현재 ${a.dashN}명) — 벤 수만큼 독이 발린다`, f:a=>a.dashN+=2},
  {id:'clone', vis:1, n:'그림자 분신', max:3, r:1, when:'기본 공격', ok:a=>a.trait==='assassin',
   d:a=>`기본 공격 뒤에 분신 ${a.clones+1}기가 서로 다른 길로 따라 벤다 — 피해는 30%, 독은 온전히 바른다`, f:a=>a.clones++},
  {id:'wide', vis:1,  n:'폭발 확대',   max:5, r:1, when:'상시', ok:a=>a.trait==='blast',
   d:a=>`이 무기 폭발 반경 +12% (현재 ${Math.round(a.blastR)})`, f:a=>a.blastR*=1.12},
  {id:'chain', vis:1, n:'불티 번짐',   max:3, r:1, when:'기본 공격', ok:a=>a.trait==='blast',
   d:a=>`폭발이 가장 가까운 적에게 ${a.chain+1}번 옮겨붙는다 — 옮길 때마다 피해가 55%로 준다`, f:a=>a.chain++},
  {id:'pierce', vis:1,n:'꿰뚫는 촉',   max:4, r:1, when:'기본 공격', ok:a=>a.trait==='shoot',
   d:a=>`화살 한 발이 ${a.pierce+2}명을 꿰뚫는다 — 한 명 뚫을 때마다 피해가 62%로 준다`, f:a=>a.pierce++},
  {id:'twin', vis:1,  n:'쌍촉 화살',   max:3, r:1, when:'기본 공격', ok:a=>a.trait==='shoot',
  ...
  {id:'ultwide', vis:1, n:'퍼지는 오의', max:3, r:1, when:'필살기', ok:a=>a.trait!=='shoot',
  ...
  {id:'rune', vis:1,  n:'파열의 룬',   max:3, r:2, when:'처치 시', ok:()=>1,
   d:a=>`이 무기로 처치한 적이 터진다 — 주위 34 안에 이 무기 공격력의 40% 피해 (${a.deathBlast+1}중첩)`, f:a=>a.deathBlast++},
  {id:'shatter', vis:1, n:'서리 파쇄', max:2, r:2, when:'적중 시', ok:()=>allies.some(x=>x.elCold),
  ...
  {id:'coldcut', n:'한파의 날', max:2, r:2, when:'상시', ok:()=>allies.some(x=>x.elCold),
   ...
   f:a=>a.syColdcut++},
  {id:'firespread', vis:1, n:'불길 전파', max:2, r:2, when:'처치 시', ok:a=>a.elFire>0,
  ...
  {id:'dotamp', n:'스며드는 고통', max:2, r:2, when:'상시', ok:a=>a.elFire>0||a.elPois>0,
   ...
   f:a=>a.syDotamp++},
  {id:'reso', vis:1, n:'공명 파열', max:2, r:2, when:'처치 시', ok:()=>allies.some(x=>x.elShock),
   d:a=>`이 무기로 공명 중인 적을 처치하면 폭발 — 주위 70 안에 이 무기 공격력의 ${Math.round(STATUS.syn.resoPct*100*(a.syReso+1))}% 피해 + 공명 전파 4마리 (누가 걸었든, 연쇄마다 55% 감쇠 · 최대 3연쇄)`,
   f:a=>a.syReso++},
  ...
  {id:'dblult', vis:1, n:'이중 오의', max:5, r:2, when:'필살기', ok:()=>1,
